...
def classify_triangle(p1, p2, p3):
    ...
    return (
        TriangleType.ACUTE if (sides[0] + sides[1]) > sides[2] else TriangleType.OBTUSE
    )
...
def _shift_sample(base_array, target_midi, base_midi, gain=1.0):
    ...
    if base_array.ndim == 1:
        ...
        shifted_f = np.stack(
            [
                np.interp(new_pos, old_pos, base_array[:, ch])
                for ch in range(base_array.shape[1])
            ],
            axis=1,
        )

    if gain != 1.0:
        shifted_f = shifted_f * gain

    return shifted_f.astype(np.float32)


def _apply_fade_out(arr_f, fade_ms, sample_rate):

    fade_samples = int(fade_ms * sample_rate / 1000)
    fade_samples = min(fade_samples, arr_f.shape[0])
    if fade_samples > 0:
        ramp = np.linspace(1.0, 0.0, fade_samples, dtype=np.float32)
        if arr_f.ndim == 1:
            arr_f[-fade_samples:] *= ramp
        else:
            arr_f[-fade_samples:, 0] *= ramp
            arr_f[-fade_samples:, 1] *= ramp
    return arr_f
...
def play_shifted_sample(
    ...
    try:
        ...
        if shifted_f.shape[0] > max_samples:
            shifted_f = shifted_f[:max_samples]

        _apply_fade_out(shifted_f, fade_out_ms, SAMPLE_RATE)

        shifted = np.clip(shifted_f, -32768, 32767).astype(np.int16)
...
def render_song_to_wav(timeline_items, out_path, song_duration_sec, max_block_sec=1.2):
...
class TriangleMusicApp:
    ...
    def __init__(self, root):
        ...
        self.view.on_bpm_change = self.on_bpm_change
        self.view.on_volume_change = self.on_volume_change
    ...
    def end_drag(self, event):
        ...
        if (
            0 <= rel_x_visible <= self.timeline.winfo_width()
            and 0 <= rel_y <= self.timeline_height
        ):
    ...
    def add_to_timeline(self, sound, track, time_sec):
        ...
        item = {
            "sound": sound,
            "track": track,
            "time_sec": x / self.PIXELS_PER_SEC,
            "rect_id": rect_id,
            "text_id": text_id,
            "tag": tag,
        }
        ...
        self.timeline.tag_bind(
            tag, "<Button-3>", lambda e, it=item: self.remove_timeline_item(it)
        )
        self.timeline.tag_bind(
            tag, "<ButtonPress-1>", lambda e, it=item: self.start_block_drag(e, it)
        )
        self.timeline.tag_bind(tag, "<B1-Motion>", self.do_block_drag)
        self.timeline.tag_bind(tag, "<ButtonRelease-1>", self.end_block_drag)
    ...
    def _start_timeline_from(self, start_sec):
        ...
        tempo_scale = 120.0 / self.view.get_bpm()

        for item in self.timeline_items:
            t = item["time_sec"]
            if t < start_sec - 0.001:
                continue
            delay_ms = int((t - start_sec) * 1000 * tempo_scale)
            s = item["sound"]
            aid = self.root.after(
                delay_ms,
                lambda snd=s: threading.Thread(
        ...
        self.playback_started_at = time.perf_counter() - start_sec * tempo_scale
        self.pause_elapsed = 0.0
    ...
    def pause_timeline(self):
        ...
        wall_elapsed = time.perf_counter() - self.playback_started_at
        ts = self._tempo_scale if self._tempo_scale > 0 else 1.0
        self.pause_elapsed = wall_elapsed / ts
        self.pause_elapsed = max(
            0.0, min(self.pause_elapsed, float(self.TIMELINE_SECONDS))
        )
        self.is_paused = True
    ...
    def _stop_playback_state(self):
        ...
        if self.playhead_id is not None:
            try:
                self.timeline.delete(self.playhead_id)
            except Exception:
                pass
            self.playhead_id = None
    ...
    def _start_playhead(self, from_sec=0.0):
        ...
        self.timeline.coords(self.playhead_id, x, 0, x, self.timeline_height)
        ts = self._tempo_scale if self._tempo_scale > 0 else 1.0
        self.playhead_start_time = time.perf_counter() - from_sec * ts
        self._tick_playhead()

    def _loop_end_sec(self):

        block_dur = self.BLOCK_WIDTH / self.PIXELS_PER_SEC
        if not self.timeline_items:
            visual_end = float(self.TIMELINE_SECONDS)
        else:
            last_end = max(it["time_sec"] for it in self.timeline_items) + block_dur
            visual_end = float(math.ceil(last_end))
        return visual_end * self._tempo_scale

    def _tick_playhead(self):
        if self.playhead_id is None or self.is_paused:
            return
        dt = time.perf_counter() - self.playhead_start_time
        loop_end = self._loop_end_sec()
        if dt >= loop_end:
            self._start_timeline_from(0.0)
            return
        ...
        ts = self._tempo_scale if self._tempo_scale > 0 else 1.0
        visual_sec = dt / ts
        x = visual_sec * self.PIXELS_PER_SEC
        self.timeline.coords(self.playhead_id, x, 0, x, self.timeline_height)
    ...
    def on_volume_change(self, volume_pct):

        self.VOLUME = volume_pct / 100.0

    def save_song(self):
        if not self.timeline_items:
            messagebox.showinfo("SoundGeometry", "Timeline is empty — nothing to save.")
            return
        ...
        def _worker():
            ...
            try:
                self.root.after(
                    0,
                    lambda: self.view.btn_save.configure(
                        text="SAVING…", state="disabled"
                    ),
                )
                render_song_to_wav(self.timeline_items, path, duration)
                self.root.after(
                    0, lambda: messagebox.showinfo("SoundGeometry", f"Saved:\n{path }")
                )
                ...
                self.root.after(
                    0,
                    lambda: self.view.btn_save.configure(text=old_text, state="normal"),
                )
